...
def create_background(cap, num_frames=30):  # you can customize 'num_frames', more frames= high resolution, but slower
    print(f"Capturing background using {num_frames} frames. Please move out of frame.")
    backgrounds = []
    for i in range(num_frames):
        ret, frame = cap.read()
        if ret:
            backgrounds.append(frame)
            print(f"Captured frame {i+1}/{num_frames}", end='\n')
        else:
            print(f"\nWarning: Could not read frame {i+1}/{num_frames}")
        time.sleep(0.1)
